...
export const RecentScenarios = ({ onNavigate }: { onNavigate?: (view: string) => void }) => {
  ...
  useEffect(() => {
    const fetchScenarios = async () => {
      try {
        ...
        setScenarios(
          dashboardStats.recentChapters?.map((ch: any) => ({
            id: ch.id,
            title: ch.title,
            sceneId: ch.sceneId,
            status: ch.status,
  ...
  return (
    <div className="col-span-1 md:col-span-1 lg:col-span-2 rounded-2xl bg-white dark:bg-surface-dark border border-gray-200 dark:border-white/5 p-0 shadow-sm overflow-hidden flex flex-col">
      <div className="p-6 pb-2 flex justify-between items-center">
        ...
        <a
          href="#"
          className="text-xs font-bold text-primary dark:text-[#4fd1d9] hover:underline"
          onClick={e => {
            e.preventDefault();
            onNavigate && onNavigate('chapterList');
          }}
        >
          View All
        </a>
