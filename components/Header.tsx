...
export const Header: React.FC<HeaderProps> = ({
  ...
  const { user, signInWithGoogle, signOutUser, isFirebaseAvailable } =
    useAuth();
  ...
  const handleExportYaml = async () => {
    try {
      const blob = await exportProjectAsYamlZip(projectData);
      const url = URL.createObjectURL(blob);
      const exportFileDefaultName = `${projectData.projectName.replace(
        /\s+/g,
        "_"
    ...
    if (file.name.endsWith(".vns")) {
      ...
      reader.onload = (e) => {
        try {
          ...
          alert(
            "Failed to import project. The file may be corrupted or not in the correct format."
          );
        }
      };
  ...
  return (
    <>
      ...
      {isIOModalOpen && (
        ...
        >
          <div
            className="bg-card rounded-lg shadow-xl border border-border p-4 m-4 max-w-sm w-full"
      ...
      {isVersionControlOpen && (
        ...
        >
          <div
            className="bg-card rounded-lg shadow-xl border border-border m-4 max-w-2xl w-full max-h-[85vh] overflow-hidden flex flex-col"
            onClick={(e) => e.stopPropagation()}
          >
      ...
      <header className="flex items-center justify-between p-2 bg-card border-b border-border shadow-md h-12 flex-shrink-0">
        <div className="flex items-center gap-2 flex-1">
          ...
          <input
            ...
            className="text-lg font-bold bg-transparent border-none text-foreground focus:outline-none focus:ring-0 w-auto"
          />
          <button
