...
export const ChatBot: React.FC<ChatBotProps> = ({ projectData }) => {
  ...
  const isAvailable = !!localStorage.getItem("apiKey");

  const summarizeEvent = useCallback(
    (event: SceneEvent): string => {
      switch (event.type) {
        case EventType.DIALOGUE:
          const char = projectData.characters.find(
        ...
        case EventType.BACKGROUND_CHANGE:
          const bg = projectData.assets.find(
            (a) =>
              a.type === AssetType.BACKGROUND &&
              a.id === event.backgroundAssetId
          );
        ...
        case EventType.SFX:
          const sfx = projectData.assets.find(
            (a) => a.type === AssetType.SFX && a.id === event.sfxAssetId
          );
  ...
  const getRelevantContext = useCallback(
    (query: string): string | null => {
      ...
      // ヘルパー: アイテムがクエリに関連するか判定
      const isRelevant = (title: string, content: string = "") => {
        const t = title.toLowerCase();
        const c = content.toLowerCase();
        
        // 1. ユーザーの質問文の中に、データのタイトルが含まれているか？ (例: "体育での怪我について" -> "体育での怪我")
        if (t && queryLower.includes(t)) return true;

        // 2. データの中に、ユーザーの検索ワードが含まれているか？ (例: "怪我" -> "体育での怪我")
        // ただし、クエリがあまりに短い場合(1文字など)はノイズになるので除外
        if (queryLower.length > 1) {
            if (t.includes(queryLower) || c.includes(queryLower)) return true;
        }
      ...
      // プロットの検索
      const matchingPlots = projectData.plots.filter((p) =>
        isRelevant(p.title, p.content)
      );

      if (matchingPlots.length > 0) {
        parts.push(
          `### Context - Detailed Plots found:\n${JSON.stringify(
            matchingPlots.map((p) => ({
              title: p.title,
              content: p.content,
            })),
      ...
      // シーンの検索
      const matchingScenes = projectData.scenes.filter((s) => {
        // タイトルチェック
        if (isRelevant(s.title)) return true;
        // イベント内容チェック
        return s.events.some(e => 
           (e.text && isRelevant("", e.text)) || 
           (e.description && isRelevant("", e.description))
        );
      });

      if (matchingScenes.length > 0) {
        const summarizedScenes = matchingScenes.map((s) => ({
          title: s.title,
          events: s.events.map(summarizeEvent),
        }));
        parts.push(
          `### Context - Detailed Scenes found:\n${JSON.stringify(
            summarizedScenes,
            null,
            2
          )}`
  ...
  const getSystemInstruction = useCallback(() => {
    const context: any = {
      ...
      locations: projectData.locations.map((l) => ({
        name: l.name,
        description: l.description,
      })),
      relationships: projectData.relationships.map((r) => ({
        source:
          projectData.characters.find((c) => c.id === r.sourceCharacterId)
            ?.name || "Unknown",
        target:
          projectData.characters.find((c) => c.id === r.targetCharacterId)
            ?.name || "Unknown",
        type: r.type,
      })),
      sceneList: projectData.scenes.map((s) => s.title),
      plotList: projectData.plots.map((p) => p.title),
    };

    return `You are a helpful and creative AI assistant for a visual novel writer.
Your role is to help the user with their project by answering questions, providing suggestions.
You have access to the user's current project structure (Characters, Locations, Lists of Scenes/Plots).
**IMPORTANT: If the user asks about a specific Scene or Plot by name, relevant detailed content will be injected into the message automatically. Please use that provided context to answer.**

Base Project Structure:
${JSON.stringify(context, null, 2)}`;
  }, [projectData]);

  const initializeGeminiInstance = useCallback(
    (history: Message[] = []) => {
  ...
  const startNewChat = useCallback(async () => {
    setMessages([
      { role: "model", content: t("aiAssistantWelcome", language) },
    ]);
  ...
  const loadChat = useCallback(
    async (id: number, allHistories: ChatHistory[]) => {
  ...
  const loadAllHistories = useCallback(async () => {
    ...
    setChatHistories(
      histories.map((h) => ({
        id: h.id,
        title:
          h.messages.find((m) => m.role === "user")?.content.substring(0, 40) ||
          t("untitledChat", language),
      }))
    );
  ...
  const handleSendMessage = async () => {
    ...
    const displayUserMessage: Message = { role: "user", content: userInput };
    const currentMessages = [...messages, displayUserMessage];
    ...
    try {
      // ユーザー入力に関連するデータを検索
      const relevantContext = getRelevantContext(originalInput);
      
      let textToSend = originalInput;

      // コンテキストが見つかった場合、メッセージに注入
      if (relevantContext) {
        textToSend = `[System: The user's query references specific project data. Use the following details to answer.]\n\n${relevantContext}\n\n[User Query]:\n${originalInput}`;
      }
      ...
      if (currentChatId === null) {
        ...
        setChatHistories((prev) => [
          { id: newId, title: originalInput.substring(0, 40) },
          ...prev,
        ]);
      } else {
  ...
  const suggestions = [
    {
      ...
      prompt: t("suggestion.character", language),
    },
          ...
          <div className="relative" ref={historyPanelRef}>
            ...
            {isHistoryOpen && (
              <div className="absolute top-full right-0 mt-2 z-20 w-72 bg-card border border-border rounded-lg shadow-xl animate-fade-in-fast">
                <div className="p-2 font-semibold text-sm border-b border-border text-foreground">
                ...
                <ul className="py-1 max-h-80 overflow-y-auto">
                  {chatHistories.length > 0 ? (
                    chatHistories.map((hist) => (
                      <li
                        key={hist.id}
                        className="group flex items-center justify-between text-sm hover:bg-secondary"
                      >
                        <button
                          onClick={() => {
                            const histories = getAllChatHistories();
                            histories.then((h) => loadChat(hist.id, h));
                          }}
                          className="flex-1 text-left px-3 py-2 truncate text-foreground"
                        >
                          {hist.title}
                        </button>
                        <button
                          ...
                          className="ml-2 mr-2 p-1 text-muted-foreground hover:text-danger opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0"
        ...
        <div className="flex-1 flex items-center justify-center p-8">
          <div className="text-center bg-background rounded-md border border-border p-6">
            <h3 className="text-lg font-semibold">
              {t("aiUnavailable", language)}
            </h3>
            <p className="text-muted-foreground mt-2">
              {t("aiUnavailableHint", language)}
            </p>
          </div>
        </div>
      ) : (
